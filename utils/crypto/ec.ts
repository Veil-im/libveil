...
export async function generateKeyPair(
  ...
  try {
    const keyUsages: KeyUsage[] =
      algorithm === CryptoAlgorithm.ECDSA ? ['sign', 'verify'] : ['deriveKey', 'deriveBits'];
...
export async function deriveSharedSecret(
  ...
  try {
    if (
      privateKey.algorithm !== CryptoAlgorithm.ECDH ||
      publicKey.algorithm !== CryptoAlgorithm.ECDH
    ) {
...
export async function deriveAESGCMKey(sharedSecret: ArrayBuffer): Promise<CryptoKey> {
  try {
    ...
    throw new Error(
      `Failed to derive AES-GCM key from shared secret: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
