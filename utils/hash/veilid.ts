...
export class VeilIdGenerator {
  ...
  public static async generate(): Promise<string> {
    const getRandomBytes = (size: number) => crypto.getRandomValues(new Uint8Array(size));

    const seed = getRandomBytes(32);
    const deviceSalt = getRandomBytes(16);

    let hash = await bcryptHash(
      Buffer.from(seed).toString('hex') + Buffer.from(deviceSalt).toString('hex')
    );
